...
def train_model(request: TrainRequest):
    ...
    if task not in ("classification", "regression", "clustering"):
        raise HTTPException(status_code=400, detail="task_type must be 'classification', 'regression', or 'clustering'.")

    if task in ("classification", "regression") and not request.target_column:
        raise HTTPException(status_code=400, detail="target_column is required for classification and regression tasks.")

    # ── Guard: continuous variable used for classification ───────────────
    ...
    # ── Guard: categorical variable used for regression ──────────────────
    ...
    try:
        X, y, preprocessor, label_encoder, feature_names = preprocess(
            df=df, task_type=task, target_column=request.target_column,
        )
    ...
    try:
        ...
        else:  # clustering — auto optimal k
            best_model, best_name, all_metrics, labels, optimal_k, k_scores = train_clustering(X)
            best_metrics = all_metrics[best_name]
            # convert k_scores keys to strings for JSON serialisation
